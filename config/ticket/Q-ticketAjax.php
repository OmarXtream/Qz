...
if(isset($_POST["msg"])){
	...
	if ( $tp == "شكوى" ) {
		if ( $admin == 'n' ) {
			$canReply = true ;
			$sql = "UPDATE ticket_system SET admin='$dbid' WHERE hash='$hash'";
		...
		if ( $admin == "n" ) {
			...
			if (!in_array($dbid, $admins)) {
			$finalResult = $admin . "," . $dbid ; 
			$sql = "UPDATE ticket_system SET admin='$finalResult' WHERE hash='$hash'";
			$result = $conn->query($sql);
			}
		}
	}
	...
	try {
		...
		$sql = "INSERT INTO ticket_notification (id, uid, msg)
		VALUES (Null,'$ccLient','$s')";
	if ($conn->query($sql) === TRUE) {
	//	echo "saved to db" ;
	}
}
