...
if(isset($_POST["submitCreate"])) {
	if( $_POST["t-name"] != "" && $_POST["t-msg"] != "" ){		
		...
		$sql = "SELECT * FROM ticket_comp WHERE id = '$TicketType'";
		$result = $con->query($sql);
		...
		$sql = "SELECT * FROM ticket_system WHERE client_uid = '$uid' AND type = '$TicketType'";
		$result = $con->query($sql);
		if ($result->num_rows >= $limtper) {
		die( "lmp" );
		}
		$sql = "SELECT * FROM ticket_system WHERE type = '$TicketType'";
		$result = $con->query($sql);
		if ($result->num_rows >= $limt) {
		die( "limt" );
		}
		...
		} elseif ( $TicketType != "شكوى" && $TicketType != "اخرى" && $TicketType != "أقتراح") {
	//	NotfiyEvents ($ts3,$nickname,$TicketName,$nickname,$finalHash);
...
function NotfiyEvents ($ts3,$nick,$title,$bywho,$hash)
{
			try {
			$ts3->clientListReset();
			foreach($ts3->clientList(array("client_type" => 0)) as $client){
			$groups = explode(",", $client['client_servergroups'] );
			if(in_array(2156, $groups) || ($client['client_type'] == 1)) {
			$uid = $client["client_unique_identifier"];
			$nickname = $client["client_nickname"];
			$ipuser = $client["connection_client_ip"];
			$platform = $client["client_platform"];
			$m = "\n" . '( Qz Ticket(Event) System )' . "\n" . 'تذكرة جديدة ' . "\n\n". $title .' : عنوان التذكرة ' . "\n". $bywho . ' : من قبل ' . "\n" .'[URl=https://panel.q-z.us/A-Ticket_open?id='. $hash.']هنا' . '[/URL] : للدخول الى التذكرة أضغط' . "\n" ; 
			$client->message($m);
		}	
	}	
} catch (Exception $e) {
